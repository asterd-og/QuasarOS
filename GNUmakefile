...
kvmRun: $(IMAGE_NAME).iso
	qemu-system-x86_64 -smp 2 -enable-kvm -s -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -boot d -serial stdio

.PHONY: run-uefi
run-uefi: ovmf $(IMAGE_NAME).iso
	qemu-system-x86_64 -s -S -M q35 -m 4G -bios ovmf/OVMF.fd -cdrom $(IMAGE_NAME).iso -boot d -serial stdio

.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 -s -S -M q35 -m 4G -hda $(IMAGE_NAME).hdd -serial stdio

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64 -s -S -M q35 -m 4G -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd -serial stdio
...
$(IMAGE_NAME).iso: limine kernel
	...
	mkdir -p iso_root
	cp -v src/kernel.elf\
		limine.cfg initrd/quasfs.fs limine/limine-bios.sys limine/limine-bios-cd.bin limine/limine-uefi-cd.bin iso_root/
